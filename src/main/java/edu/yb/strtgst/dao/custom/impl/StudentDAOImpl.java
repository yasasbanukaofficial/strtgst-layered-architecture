...
public class StudentDAOImpl implements StudentDAO {
    ...
    public Student getEntity(String username) throws SQLException{
        ResultSet rst = SQLUtil.execute("SELECT * FROM Student WHERE username = ?", username);
        if (rst.next()){
            return new Student(
                    rst.getString(1),
                    rst.getString(2),
    ...
    public boolean fetchExistingUsername(String username) throws SQLException{
        ResultSet rst = SQLUtil.execute("SELECT username FROM Student WHERE username = ?", username);
        return rst.next();
    }

    @Override
    public boolean fetchExistingEmail(String email) throws SQLException{
        ResultSet rst = SQLUtil.execute("SELECT email FROM Student WHERE email = ?", email);
        return rst.next();
    }
    ...
    public String fetchStudentName(String username) throws SQLException{
        ResultSet rst = SQLUtil.execute("SELECT stud_name FROM Student WHERE username = ?", username);
        while (rst.next()){
    ...
    public String getEntityIdByUsername(String username) throws SQLException {
        ResultSet rst = SQLUtil.execute("SELECT stud_id FROM Student WHERE username = ?", username);
        while (rst.next()){
            return rst.getString(1);
        }
        return null;
